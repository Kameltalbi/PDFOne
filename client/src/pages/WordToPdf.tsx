...
function WordToPdf() {
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      setError('Veuillez sélectionner au moins un fichier Word');
      return;
    }
    ...
    <div className="word-to-pdf-page">
      <div className="word-to-pdf-container">
        <div className="breadcrumbs">
          ...
          <span className="current">Word en PDF</span>
        </div>

        <div className="word-to-pdf-header">
          <h1>Word en PDF</h1>
          <p>Convertissez vos documents Word en fichiers PDF</p>
        </div>

        <main className="word-to-pdf-content">
          {!downloadUrl ? (
            <>
              <FileUpload 
                multiple={true}
              ...
              {files.length > 0 && (
                <div className="word-to-pdf-actions">
                  ...
                  >
                    {isProcessing ? 'Conversion en cours...' : `Convertir ${files.length} fichier${files.length > 1 ? 's' : ''}`}
                  </button>
                </div>
              )}
            ...
            <div className="success-container">
              ...
              <p>Vos documents Word ont été convertis en PDF.</p>
              <div className="download-actions">
                <a href={downloadUrl} download="converted.pdf" className="download-button">
                  Télécharger le PDF
                </a>
